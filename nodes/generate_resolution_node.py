...
def generate_resolution_node(state: AnalyzerState) -> dict:
    ticket = state["ticket"]
    past_solutions = state["past_solutions"]
    logger.info(
        "Generating resolution",
        extra={"ticket_number": ticket.number, "past_solutions_count": len(past_solutions)},
    )

    formatted_past_solutions = "\n".join([f"- {sol}" for sol in past_solutions])
    ...
    logger.info(
        ...
        extra={"ticket_number": ticket.number, "plan_length": len(final_plan)},
    )
